...
async function getScope(platformConn: any, booksConn: any, userId: string, organisationId: string, permission?: string): Promise<TenantScope> {
  ...
  // Non-admins (or if no org matched above) check their organization membership in Books DB
  const [rows]: any = await booksConn.query(
  ...
  if (permission && !["ORGANISATION_OWNER", "PLATFORM_ADMIN"].includes(scope.roleCode)) {
    const [grants]: any = await booksConn.query(
...
export function registerBooksRoutes(app: Express, getPlatformConnection: ConnectionFactory, getBooksConnection: ConnectionFactory): void {
  ...
  app.get("/api/portal/books/bootstrap", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      const user = await getUser(platformConn, userId);
  ...
  app.post("/api/portal/books/organisations", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      if (platformAdmin && data.sourceEntityId && !adminFirm) {
        const [entities]: any = await platformConn.query(
        ...
        const [clients]: any = await platformConn.query("SELECT id, companyName FROM Client WHERE LOWER(contactEmail) = LOWER(?) LIMIT 1", [user.email]);
        client = clients[0] || null;
        if (data.sourceEntityId) {
          const [entities]: any = await platformConn.query("SELECT e.id FROM Entity e JOIN Client c ON c.id = e.clientId WHERE e.id = ? AND LOWER(c.contactEmail) = LOWER(?) LIMIT 1", [data.sourceEntityId, user.email]);
          if (!entities.length) throw new BooksHttpError(403, "The selected entity is not available to this account", "ENTITY_ACCESS_DENIED");
        }
      }
      let tenantId = "";
      const [tenants]: any = client
        ? await booksConn.query("SELECT id FROM BooksTenant WHERE clientId = ? AND status = 'ACTIVE' LIMIT 1 FOR UPDATE", [client.id])
        : await booksConn.query("SELECT id FROM BooksTenant WHERE ownerUserId = ? AND clientId IS NULL AND status = 'ACTIVE' LIMIT 1 FOR UPDATE", [userId]);
      ...
      const ownerRoleId = await seedOrganisation(booksConn, tenantId, organisationId);
      await booksConn.query("INSERT INTO BooksOrganisationMember (id, tenantId, organisationId, userId, roleId, status) VALUES (?, ?, ?, ?, ?, 'ACTIVE')", [makeId("bom"), tenantId, organisationId, userId, ownerRoleId]);
      const branchId = makeId("bb");
      const stateCode = data.gstin?.slice(0, 2) || data.stateCode || null;
      await booksConn.query("INSERT INTO BooksBranch (id, tenantId, organisationId, name, code, stateCode, address, isHeadOffice) VALUES (?, ?, ?, 'Head Office', 'HO', ?, ?, 1)", [branchId, tenantId, organisationId, stateCode, data.registeredAddress ? JSON.stringify(data.registeredAddress) : null]);
      if (data.gstin && stateCode) await booksConn.query("INSERT INTO BooksGSTRegistration (id, tenantId, organisationId, branchId, gstin, legalName, tradeName, stateCode, isDefault) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)", [makeId("bgst"), tenantId, organisationId, branchId, data.gstin, data.legalName, data.tradeName || null, stateCode]);
      const year = currentFiscalYear(data.financialYearStartMonth);
      const fiscalYearId = makeId("bfy");
      await booksConn.query("INSERT INTO BooksFiscalYear (id, tenantId, organisationId, label, startsOn, endsOn, isCurrent) VALUES (?, ?, ?, ?, ?, ?, 1)", [fiscalYearId, tenantId, organisationId, year.label, year.startsOn, year.endsOn]);
      for (const period of monthlyPeriods(year.startsOn)) await booksConn.query("INSERT INTO BooksAccountingPeriod (id, tenantId, organisationId, fiscalYearId, name, startsOn, endsOn) VALUES (?, ?, ?, ?, ?, ?, ?)", [makeId("bap"), tenantId, organisationId, fiscalYearId, period.name, period.startsOn, period.endsOn]);
      const scope: TenantScope = { tenantId, organisationId, userId, roleCode: "ORGANISATION_OWNER" };
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "organisation.created", entityType: "organisation", entityId: organisationId, afterData: { legalName: data.legalName, entityType: data.entityType, sourceEntityId: data.sourceEntityId || null }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
  ...
  app.get("/api/portal/books/dashboard", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "dashboard.view");
      const params = [scope.tenantId, scope.organisationId];
      const [[sales]]: any = await booksConn.query(
      ...
      const [[purchases]]: any = await booksConn.query(
      ...
      const [[cash]]: any = await booksConn.query(
      ...
      const [[gst]]: any = await booksConn.query(
        `SELECT COALESCE(SUM(cgstTotal),0) outputCgst, COALESCE(SUM(sgstTotal),0) outputSgst, COALESCE(SUM(igstTotal),0) outputIgst
         FROM BooksInvoice WHERE tenantId = ? AND organisationId = ? AND status NOT IN ('DRAFT','CANCELLED','VOID')`, params);
      const [activity]: any = await booksConn.query("SELECT id, action, entityType, entityId, createdAt FROM BooksAuditLog WHERE tenantId = ? AND organisationId = ? ORDER BY createdAt DESC LIMIT 8", params);
      const [cashFlow]: any = await booksConn.query(
      ...
      const [orgRows]: any = await booksConn.query("SELECT sourceEntityId FROM BooksOrganisation WHERE tenantId = ? AND id = ? LIMIT 1", params);
      const sourceEntityId = orgRows[0]?.sourceEntityId;
      let compliance: any[] = [];
      if (sourceEntityId) {
        const [taskRows]: any = await platformConn.query(
          "SELECT id, title, category, dueDate, status FROM ComplianceTask WHERE entityId = ? AND status <> 'COMPLETED' ORDER BY dueDate LIMIT 5",
          [sourceEntityId]
        );
        compliance = taskRows;
      }
  ...
  app.get("/api/portal/books/contacts", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "contacts.view");
      ...
      const [contacts]: any = await booksConn.query(
  ...
  app.post("/api/portal/books/contacts", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, data.organisationId, "contacts.manage");
      const id = makeId("bco");
      await booksConn.beginTransaction();
      await booksConn.query(
      ...
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "contact.created", entityType: "contact", entityId: id, afterData: data, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.get("/api/portal/books/items", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "items.view");
      const [items]: any = await booksConn.query(
      ...
      const [units]: any = await booksConn.query("SELECT id, code, name FROM BooksUnit WHERE tenantId = ? AND organisationId = ? AND isActive = 1 ORDER BY name", [scope.tenantId, scope.organisationId]);
      const [taxRates]: any = await booksConn.query("SELECT id, name, rate FROM BooksTaxRate WHERE tenantId = ? AND organisationId = ? AND isActive = 1 ORDER BY rate", [scope.tenantId, scope.organisationId]);
  ...
  app.post("/api/portal/books/items", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, data.organisationId, "items.manage");
      const [master]: any = await booksConn.query(
      ...
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "item.created", entityType: "item", entityId: id, afterData: data, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.get("/api/portal/books/invoices", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "sales.view");
  ...
  app.post("/api/portal/books/invoices", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, data.organisationId, "sales.manage");
      await booksConn.beginTransaction();
      const [customers]: any = await booksConn.query(
      ...
      const [orgRows]: any = await booksConn.query("SELECT invoicePrefix, nextInvoiceNumber FROM BooksOrganisation WHERE id = ? AND tenantId = ? FOR UPDATE", [scope.organisationId, scope.tenantId]);
      const org = orgRows[0];
      const invoiceNumber = `${org.invoicePrefix}-${String(org.nextInvoiceNumber).padStart(5, "0")}`;
      await booksConn.query("UPDATE BooksOrganisation SET nextInvoiceNumber = nextInvoiceNumber + 1 WHERE id = ? AND tenantId = ?", [scope.organisationId, scope.tenantId]);
      let branchId = data.branchId || "";
      if (!branchId) {
        const [defaultBranches]: any = await booksConn.query("SELECT id FROM BooksBranch WHERE tenantId = ? AND organisationId = ? AND isHeadOffice = 1 AND isActive = 1 LIMIT 1", [scope.tenantId, scope.organisationId]);
        branchId = defaultBranches[0]?.id;
      }
      const [branches]: any = await booksConn.query("SELECT id, stateCode FROM BooksBranch WHERE id = ? AND tenantId = ? AND organisationId = ? AND isActive = 1 LIMIT 1", [branchId, scope.tenantId, scope.organisationId]);
      ...
      await ensureConfiguredTaxRates(booksConn, scope, data.lines.map((line) => line.gstRate));
      ...
      for (const line of calculated) {
        if (line.itemId) {
          const [items]: any = await booksConn.query("SELECT id FROM BooksItem WHERE id = ? AND tenantId = ? AND organisationId = ? AND isActive = 1 LIMIT 1", [line.itemId, scope.tenantId, scope.organisationId]);
      ...
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "invoice.draft_created", entityType: "invoice", entityId: invoiceId, afterData: { invoiceNumber, customerId: data.customerId, grandTotal }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.post("/api/portal/books/invoices/:id/post", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "sales.post");
      await booksConn.beginTransaction();
      const [invoices]: any = await booksConn.query(
      ...
      const fiscalYearId = await requireOpenPeriod(booksConn, scope, entryDate);
      ...
      const journalId = await insertJournal(booksConn, {
      ...
      await booksConn.query("UPDATE BooksInvoice SET status = 'POSTED', postedAt = NOW(3), postedBy = ? WHERE id = ? AND tenantId = ? AND organisationId = ?", [userId, invoice.id, scope.tenantId, scope.organisationId]);
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "invoice.posted", entityType: "invoice", entityId: invoice.id, beforeData: { status: "DRAFT" }, afterData: { status: "POSTED", journalId }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.post("/api/portal/books/payments", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, data.organisationId, "sales.post");
      await booksConn.beginTransaction();
      const [invoices]: any = await booksConn.query(
      ...
      const [deposit]: any = await booksConn.query("SELECT id FROM BooksAccount WHERE id = ? AND tenantId = ? AND organisationId = ? AND type = 'ASSET' AND isActive = 1 LIMIT 1", [depositAccountId, scope.tenantId, scope.organisationId]);
      if (!deposit.length) throw new BooksHttpError(400, "Selected deposit account is not available");
      await booksConn.query("SELECT id FROM BooksOrganisation WHERE id = ? AND tenantId = ? FOR UPDATE", [scope.organisationId, scope.tenantId]);
      const [countRows]: any = await booksConn.query("SELECT COUNT(*) count FROM BooksCustomerPayment WHERE tenantId = ? AND organisationId = ?", [scope.tenantId, scope.organisationId]);
      ...
      await booksConn.query("INSERT INTO BooksPaymentAllocation (id, tenantId, organisationId, paymentId, invoiceId, amount) VALUES (?, ?, ?, ?, ?, ?)", [makeId("bpa"), scope.tenantId, scope.organisationId, paymentId, invoice.id, data.amount]);
      const allocation = allocateCustomerPayment(String(invoice.balanceDue), String(invoice.amountPaid), data.amount);
      await booksConn.query("UPDATE BooksInvoice SET amountPaid = ?, balanceDue = ?, status = ? WHERE id = ? AND tenantId = ? AND organisationId = ?", [allocation.amountPaid, allocation.balanceDue, allocation.status, invoice.id, scope.tenantId, scope.organisationId]);
      ...
      const journalId = await insertJournal(booksConn, { scope, fiscalYearId, sourceType: "CUSTOMER_PAYMENT", sourceId: paymentId, entryNumber: `JE-${paymentNumber}`, entryDate: data.paymentDate, narration: `Payment for ${invoice.invoiceNumber}`, userId, lines });
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "payment.posted", entityType: "customer_payment", entityId: paymentId, afterData: { paymentNumber, amount: data.amount, invoiceId: invoice.id, journalId }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.get("/api/portal/books/bills", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      const [bills]: any = await booksConn.query(
        ...
        [scope.tenantId, scope.organisationId]);
      const [expenses]: any = await booksConn.query(
  ...
  app.post("/api/portal/books/bills", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, data.organisationId, "purchases.manage");
      await booksConn.beginTransaction();
      const [vendors]: any = await booksConn.query("SELECT id FROM BooksContact WHERE id = ? AND tenantId = ? AND organisationId = ? AND type IN ('VENDOR','BOTH') AND isActive = 1 LIMIT 1", [data.vendorId, scope.tenantId, scope.organisationId]);
      if (!vendors.length) throw new BooksHttpError(400, "Selected vendor is not available to this organisation");
      const [branches]: any = await booksConn.query("SELECT id, stateCode FROM BooksBranch WHERE tenantId = ? AND organisationId = ? AND isHeadOffice = 1 AND isActive = 1 LIMIT 1", [scope.tenantId, scope.organisationId]);
      ...
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "bill.draft_created", entityType: "bill", entityId: billId, afterData: { billNumber: data.billNumber, vendorId: data.vendorId, grandTotal }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.post("/api/portal/books/bills/:id/post", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "purchases.post");
      await booksConn.beginTransaction();
      const [bills]: any = await booksConn.query("SELECT id, vendorId, billNumber, billDate, status, subTotal, inputCgst, inputSgst, inputIgst FROM BooksBill WHERE id = ? AND tenantId = ? AND organisationId = ? FOR UPDATE", [req.params.id, scope.tenantId, scope.organisationId]);
      ...
      const journalId = await insertJournal(booksConn, { scope, fiscalYearId, sourceType: "VENDOR_BILL", sourceId: bill.id, entryNumber: `JE-BILL-${bill.id.slice(-8)}`, entryDate, narration: `Vendor bill ${bill.billNumber}`, userId, lines });
      await booksConn.query("UPDATE BooksBill SET status = 'POSTED', postedAt = NOW(3), postedBy = ? WHERE id = ? AND tenantId = ? AND organisationId = ?", [userId, bill.id, scope.tenantId, scope.organisationId]);
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "bill.posted", entityType: "bill", entityId: bill.id, beforeData: { status: "DRAFT" }, afterData: { status: "POSTED", journalId }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.post("/api/portal/books/expenses", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      const fiscalYearId = await requireOpenPeriod(booksConn, scope, data.expenseDate);
      const accounts = await accountMap(booksConn, scope);
      ...
      const journalId = await insertJournal(booksConn, { scope, fiscalYearId, sourceType: "EXPENSE", sourceId: expenseId, entryNumber: `JE-EXP-${expenseId.slice(-8)}`, entryDate: data.expenseDate, narration: data.description, userId, lines });
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "expense.posted", entityType: "expense", entityId: expenseId, afterData: { description: data.description, amount: data.amount, journalId }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit();
  ...
  app.get("/api/portal/books/reports/financials", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "reports.view");
      const [profitLoss]: any = await booksConn.query(
      ...
      const [balanceSheet]: any = await booksConn.query(
        `SELECT a.id, a.code, a.name, a.type,
  ...
  app.get("/api/portal/books/gst/summary", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      const [[outward]]: any = await booksConn.query(
      ...
      const [[inward]]: any = await booksConn.query(
  ...
  app.get("/api/portal/books/periods", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "accountant.view");
      const [periods]: any = await booksConn.query("SELECT id, name, startsOn, endsOn, status, lockedAt, lockedBy FROM BooksAccountingPeriod WHERE tenantId = ? AND organisationId = ? ORDER BY startsOn", [scope.tenantId, scope.organisationId]);
      res.json({ periods });
    } catch (error) { respondError(res, error); } finally { platformConn?.release(); booksConn?.release(); }
  });
  ...
  app.post("/api/portal/books/periods/:id/lock", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "accountant.lock_period");
      await booksConn.beginTransaction();
      const [periods]: any = await booksConn.query("SELECT id, name, startsOn, endsOn, status FROM BooksAccountingPeriod WHERE id = ? AND tenantId = ? AND organisationId = ? FOR UPDATE", [req.params.id, scope.tenantId, scope.organisationId]);
      ...
      await booksConn.query("UPDATE BooksAccountingPeriod SET status = 'LOCKED', lockedAt = NOW(3), lockedBy = ? WHERE id = ? AND tenantId = ? AND organisationId = ?", [userId, req.params.id, scope.tenantId, scope.organisationId]);
      await writeAudit(booksConn, { scope, actorUserId: userId, action: "accounting_period.locked", entityType: "accounting_period", entityId: req.params.id, beforeData: { status: periods[0].status }, afterData: { status: "LOCKED", name: periods[0].name }, ipAddress: req.ip, userAgent: req.headers["user-agent"] || null });
      await booksConn.commit(); res.json({ period: { ...periods[0], status: "LOCKED" } });
    } catch (error) { if (booksConn) try { await booksConn.rollback(); } catch {}; respondError(res, error); } finally { platformConn?.release(); booksConn?.release(); }
  });
  ...
  app.get("/api/portal/books/audit", async (req: PortalRequest, res) => {
    ...
    try {
      ...
      platformConn = await getPlatformConnection();
      booksConn = await getBooksConnection();
      const scope = await getScope(platformConn, booksConn, userId, organisationId, "audit.view");
      const [entries]: any = await booksConn.query(
